...
const mostrarCatalogo = (arrayCat) => {
    ...
    arrayCat.forEach((catProd) => {
        ...
        <button onclick="agregarCarrito(${catProd.codArt})" class="boton-agregar">viajar <i class="fas fa-shopping-cart"></i></button>
        `
...
const buscar = (search) => {
    return guiaViajes.filter((prod) => prod.ciudad.toLowerCase().includes(search))
}
